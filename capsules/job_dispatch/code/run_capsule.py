...
if __name__ == "__main__":
    ...
    if INPUT == "aind":
        ...
        # Probe/device names must survive serialization for combined NWB export.
        include_annotations = True
        ...
        for probe_paths_session, session_name, reader_kwargs in zip(probe_paths, session_names, reader_kwargs_list):
            is_neo_reader = issubclass(recording_extractor_full_dict[reader_type], NeoBaseRecordingExtractor)
            if is_neo_reader:
            ...
            for block_index in range(num_blocks):
                if is_neo_reader:
                    reader_kwargs["block_index"] = block_index
                for stream_name in stream_names:
                    ...
                    if probe_path is not None:
                        ...
                        # SI 0.103.2 attaches these annotations to the original
                        # recording, not the clone returned by set_probegroup.
                        recording.annotate(probes_info=[probe.annotations for probe in probegroup.probes])
            ...
            if SPLIT_GROUPS and len(np.unique(recording.get_channel_groups())) > 1:
                for group_name, recording_group in recording.split_by("group").items():
                    ...
                    job_dict = dict(
                        ...
                        recording_dict=recording_group.to_dict(recursive=True, include_annotations=include_annotations, relative_to=data_folder),
