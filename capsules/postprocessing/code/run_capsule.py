...
if __name__ == "__main__":
    ...
    for recording_name in recording_names:
        ...
        if sorting.get_num_units() == 0:
            # No unit-specific samples exist. Preserve a real analyzer and
            # its recording, with empty per-unit arrays and metric tables.
            # The pinned random sampler and spike nodes cannot reduce empty
            # arrays; selecting all (zero) spikes avoids inventing samples.
            logging.info("\tCreating explicit zero-unit analyzer")
            n_duplicated = 0
            postprocessing_notes += "\n- No spikes detected; spike and PCA nodes were not run.\n"
            recording = recording_lazy if recording_lazy is not None else recording_bin
            sorting_analyzer = si.create_sorting_analyzer(
                sorting=sorting, recording=recording, sparse=False,
                format="binary_folder", folder=scratch_folder / "tmp_analyzer",
                return_scaled=postprocessing_params["return_scaled"],
            )
            if recording_lazy is not None:
                sorting_analyzer.set_temporary_recording(recording_bin)
            analyzer_dict = postprocessing_params.copy()
            for key in ("duplicate_threshold", "return_scaled", "spike_amplitudes", "spike_locations", "principal_components"):
                analyzer_dict.pop(key, None)
            analyzer_dict["random_spikes"] = {"method": "all"}
        ...
        metrics_to_compute = quality_metrics_names
        if sorting.get_num_units() == 0:
            # These extensions produce correctly typed empty tables. Metrics
            # requiring spike/PCA nodes are undefined without any observations.
            empty_safe_metrics = {
                "num_spikes", "firing_rate", "presence_ratio", "snr", "isi_violation",
                "rp_violation", "sliding_rp_violation", "amplitude_cutoff",
                "amplitude_median", "synchrony", "firing_range",
            }
            metrics_to_compute = [name for name in quality_metrics_names if name in empty_safe_metrics]
            skipped_metrics = [name for name in quality_metrics_names if name not in empty_safe_metrics]
            postprocessing_notes += f"- Metrics not applicable to zero units: {skipped_metrics}\n"
        ...
        if sorting.get_num_units() == 0:
            postprocessing_outputs.update(status="no_spikes_detected", units=0, spikes=0)
            postprocessing_outputs["metrics_not_applicable"] = skipped_metrics
