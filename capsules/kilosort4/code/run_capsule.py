...
def detected_no_spikes(sorter_log):
    """Recognize only Kilosort's completed, zero-spike detection outcome."""
    error_trace = sorter_log.get("error_trace", [])
    runtime_trace = sorter_log.get("runtime_trace", [])
    return (
        sorter_log.get("sorter_name") == "kilosort4"
        and sorter_log.get("error") is True
        and bool(error_trace)
        and error_trace[-1].strip() == "ValueError: No spikes detected, cannot continue sorting."
        and any(re.search(r"\b0 spikes extracted in ", line) for line in runtime_trace)
        and any("st0 shape: (0, 6)" in line for line in runtime_trace)
    )
...
if __name__ == "__main__":
    ...
    for recording_folder in preprocessed_folders:
        ...
        try:
            ...
            if detected_no_spikes(spike_sorter_log):
                # Zero detections are a scientific result, not a failed CUDA
                # run. Keep the original error log and a loadable empty sorting.
                logging.info("Detection completed with zero spikes; saving an explicit empty sorting")
                empty = np.array([], dtype="int64")
                sorting = si.NumpySorting.from_samples_and_labels(
                    [empty], [empty], recording.get_sampling_frequency(), unit_ids=empty
                )
                if split_segments:
                    sorting = si.split_sorting(sorting, recording)
                sorting.annotate(sorting_status="no_spikes_detected")
                sorting = sorting.save(folder=sorting_output_folder)
                shutil.copy(log_file, sorting_output_folder)
                sorting_params = json.loads(
                    (spikesorted_raw_output_folder / recording_name / "spikeinterface_params.json").read_text()
                )
                sorting_outputs = dict(status="no_spikes_detected", units=0, spikes=0, empty_units=0)
                status = dict(
                    **sorting_outputs, sample_rate=recording.get_sampling_frequency(),
                    frames=[recording.get_num_frames(i) for i in range(recording.get_num_segments())],
                    detector_log="spikeinterface_log.json",
                )
                (sorting_output_folder / "sorting_status.json").write_text(json.dumps(status, indent=2))
                spikesorting_notes += "\n- Detection completed with zero spikes; no units were inferred.\n"
            elif RAISE_IF_FAILS:
                raise Exception(e)
            else:
                # save log to results
